...
/* =====================================================================
   图片资源库（IMG）—— 旅行照片，作为图片票根的图 / 视频票根的封面帧
   后期替换：直接修改下方 URL 即可
   ===================================================================== */
const IMG = {
    // 京都 · 雨后传统町屋老街，和服行人、纸灯笼、湿润石板路
...
/* =====================================================================
   视频资源库（VID）—— 演示用公共示例视频（mp4，跨域友好）
   说明：当前无旅行视频生成能力，此处用 Google 官方 sample 视频占位，
        后期请替换为真实旅行视频 URL；poster 仍用上方 IMG 旅行照作封面。
   ===================================================================== */
const VID = {
    // 占位视频 A（约 15s 短片）
...
/* =====================================================================
   ...
   字段说明：
     id       — 唯一编号
     display  — 票根上显示的地点文字（保留旧打字机票根的拼写质感）
     real     — 真实地名（仅供维护参考）
     date     — 日期
     type     — 媒体类型：'image' 静态照片 | 'video' 视频
     image    — type='image' 时的照片地址（引用 IMG）
     video    — type='video' 时的视频地址（引用 VID）
     poster   — type='video' 时的封面帧地址（墙上缩略图 + 播放器海报，引用 IMG）
     lat,lng  — 打卡点经纬度（数值；墙上显示两位小数，放大显示四位小数，
                方向由正负自动推导：lat≥0→N 否则 S，lng≥0→E 否则 W）
     note     — 放大查看时显示的旅行备注
   增 / 删 / 改：直接编辑本数组；顺序即墙上排列顺序
   ===================================================================== */
const TICKETS = [
    /* ---------- 第一行 ---------- */
...
/* =====================================================================
   工具：经纬度数值 → 显示字符串
   wall=true 两位小数（墙上紧凑），否则四位小数（放大完整）
   ===================================================================== */
...
/* 墙上缩略媒体 HTML：image→img；video→poster 图 + 播放徽章 */
...
function openDrawer(){
    drawer.classList.add('open'); drawerBackdrop.classList.add('open'); menuBtn.classList.add('open');
    menuBtn.setAttribute('aria-expanded', 'true'); drawer.setAttribute('aria-hidden', 'false');
    document.body.style.overflow = 'hidden';
}
function closeDrawer(){
    drawer.classList.remove('open'); drawerBackdrop.classList.remove('open'); menuBtn.classList.remove('open');
